class Company {
    ...
    addEmployee(username, salary, position, department) {
        ...
        let employee = { username: username, salary: salary, position: position, department: department };
        ...
        if (this.departments.filter(function (e) { return e.name === department; }).length > 0) {
            for (let existingDepartment of this.departments) {
                if (existingDepartment.name === department) {
                    existingDepartment.users.push(employee);
            ...
            let newDepartment = {
                name: department, users: [employee],
                totalSalary: salary, averageSalary() { return this.totalSalary / this.users.length; }
            };
